...
impl Message {
    pub fn read(kind: u8, buf: &[u8]) -> anyhow::Result<Message> {
        let len = buf.len();
        let mut cur = Cursor::new(buf);
        match (kind, len) {
            (0x11, _) => Ok(Message::Frame(CANMessage::read(&mut cur)?)),
            (0x14, 1) => Ok(Message::Reset {
                cause: ReadBytesExt::read_u8(&mut cur)?,
            }),
            ...
            (0x18, _) => Ok(Message::FirmwareIdResponse(
...
impl fmt::Debug for Message {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Message::Ping => f.write_str("Message::Ping"),
            Message::Reset { cause } => {
                let mut tuple = f.debug_struct("Message::Reset");
                tuple.field("cause", cause);
                tuple.finish()
            }
            Message::Resync => f.write_str("Message::Resync"),
            Message::VersionRequest => f.write_str("Message::VersionRequest"),
            Message::VersionReply { major, minor } => {
            ...
            Message::FirmwareIdRequest => f.write_str("Message::FirmwareIdRequest"),
            ...
            Message::BusPowerRequest => f.write_str("Message::BusPowerRequest"),
            Message::BusPowerResponse {
                v,
                i,
                reference,
...
pub async fn write_packet_to_cand_async<W: AsyncWrite + Unpin>(
    w: &mut W,
    msg: &Message,
) -> Result<()> {
